...
export default function FamilyTreeActions({ onAddFamily, families }) {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        // Gọi API lấy toàn bộ danh sách thú cưng
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/pets/codes/all`
        );
        const data = await res.json();
        console.log("✅ fetchInitialData - pet list:", data);

        setCodeParentOptions(data); // Gán danh sách cho dropdown nếu cần

        // Tạo mã code dựa vào độ dài của danh sách
        const nextNumber = data.length + 1; // Tăng thêm 1 so với số hiện có
        const paddedCode = `SM${nextNumber.toString().padStart(2, "0")}`; // SM01, SM02, ...

        setCode(paddedCode);
      } catch (err) {
        console.error("❌ Lỗi khi fetch code-name:", err.message);
        setCode("SM01"); // fallback nếu lỗi
      }
    };
